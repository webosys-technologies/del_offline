...
class Centers_model extends CI_Model
{
    ...
    {
        $this->db->from('centers as cen');  
        $this->db->join('cities as ci','ci.cityID=cen.center_city','LEFT');
        $this->db->join('states as st','st.stateID=cen.center_state','LEFT');      
        $query=$this->db->get();
        return $query->result();

    }
     public function center_result_by_where($where)
    {
        $this->db->from('centers as cen');  
        $this->db->join('cities as ci','ci.cityID=cen.center_city','LEFT');
        $this->db->join('states as st','st.stateID=cen.center_state','LEFT');  
